...
const InvoiceTab = (props) => {
  ...
  const client = selectClient(props.order.clientId)
  return (
          ...
          <CardContent className="p-88 print:p-0">
            ...
            <div className="flex justify-between">
              <div>
                ...
                {/*selectClient.invoiceAddress.address && (
                  <Typography color="text.secondary">
                    {selectClient.invoiceAddress.address}
                  </Typography>
                )*/}
                {client?.phoneNumber && (
                  <Typography color="text.secondary">{client?.phoneNumber}</Typography>
                )}
                {client?.email && (
                ...
                <TableBody>
                  {props.order.items.map((product) => (
                    <TableRow key={product.id}>
                      ...
                      <TableCell align="right">{formatter.format(product.cost)}</TableCell>
                      <TableCell align="right">{product.quantity}</TableCell>
                      <TableCell align="right">{formatter.format(product.valueCost)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
