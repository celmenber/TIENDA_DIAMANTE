...
export const CartReducer = (state = initialState, action) => {
  const selectedCart = state.carts.find((cart) => cart.id === state.cartSelectedId);
  switch (action.type) {
    ...
    case TYPES.CANCEL_CART:
      state.carts.find((cart) => cart.id === action.payload).status = 'Canceled';
      return {
        ...state,
      };
    ...
    case TYPES.ADD_PRODUCT:
      !selectedCart.items.find((item) => item.id === String(action.payload.id)) &&
    ...
    case TYPES.RETIRE_PRODUCT:
      return {
        ...
        carts: [
          ...
          {
            ...selectedCart,
            items: selectedCart.items.filter((item) => item.id !== `${action.payload.id}`),
    ...
    case TYPES.INCREMENT_PRODUCT:
      ...
      const itemtoIncrement = selectedCart.items.find((item) => item.id === action.payload);
      itemtoIncrement.quantity++;
      itemtoIncrement.valueCost =
        itemtoIncrement.quantity *
        (itemtoIncrement.costDesc ? itemtoIncrement.costDesc : itemtoIncrement.cost);
      return state;
    case TYPES.DECREMENT_PRODUCT:
      const itemtoDecrement = selectedCart.items.find((item) => item.id === action.payload);
      itemtoDecrement.quantity - 1 > 0 && itemtoDecrement.quantity--;
      itemtoDecrement.valueCost =
        itemtoDecrement.quantity *
        (itemtoDecrement.costDesc ? itemtoDecrement.costDesc : itemtoDecrement.cost);
      return state;
    case TYPES.SET_ITEM_QUANTITY:
      const itemtoChange = selectedCart.items.find((item) => item.id === action.payload.id);
