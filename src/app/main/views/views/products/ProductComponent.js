...
const ProductComponent = ({
  ...
  const { addProductFromCart, retireProductFromCart, calculateTotal, selectSelectedCart } =
    useCart();

  const isInCart = selectSelectedCart.items.map((product) => product.id).includes(`${data.id}`);
    ...
    <Grid width={250} justifyContent="center" item visibility={visible ? 'visible' : 'hidden'}>
      <Card>
        <CardHeader
          ...
          action={
            <IconButton aria-label="Activo">
              {isInCart ? (
                <CheckCircleOutline sx={{ marginRight: 0, color: 'background.paper' }} />
              ) : (
        ...
        <CardMedia
          ...
          component="img"
          image={`${'https://reinbou.com.co/wp-content/uploads/2023/06/ron2-viejo-de-caldas-caja-375-ml.png'}`}
          alt="ron2-viejo-de-caldas-caja-375-ml"
          /*  //alt={data.images[0].name} */
        />
        <CardContent sx={{ paddingBottom: 0, height: 60, overflow: 'scroll' }}>
          <Typography variant="body1" color={data.stock_quantity ? 'text.primary' : 'red'}>
            {data.stock_quantity ? 'Unidades disponibles' : 'Sin Unidades disponibles'}
          </Typography>
          <Typography
            variant="body2"
            display="grid"
