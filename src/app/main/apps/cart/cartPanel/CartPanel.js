...
import { closeCartPanel, openCartPanel, selectCartPanelState } from '../store/stateSlice';
          ...
          <Toolbar className="px-4 h-10 text-smaller">
            <div className="flex flex-1 items-center px-8 space-x-12">
              <IconButton
                ...
                onClick={() => (state ? dispatch(openCartPanel) : resetProducts(selectedCart.id))}
              ...
              >
                <Badge color="secondary" badgeContent={selectedCart && selectedCart.items.length}>
                  <FuseSvgIcon size={24}>heroicons-outline:shopping-cart</FuseSvgIcon>
                </Badge>
              </IconButton>
              <Typography className="text-16" sx={{ width: 'max-content' }} color="inherit">
                {state && 'Recepcion local'}
              </Typography>
            </div>

            <div className="flex px-4">
              <IconButton onClick={(ev) => dispatch(closeCartPanel())} color="inherit" size="large">
